...
3. **개념 페이지 생성/갱신**
   ...
   - 관련 페이지 간 [[wikilink]] 상호참조를 양방향으로 갱신한다. "관련 항목" 섹션은 **다른** 페이지와의 상호참조 전용 — 참조할 다른 페이지가 없으면 섹션을 생략한다 (자기참조 금지).

4. **인덱스/로그 갱신**
   - `wiki/index.md`에 새/변경 페이지를 반영한다 (topic 섹션, 한 줄 요약).
   - `wiki/log.md`에 `## [YYYY-MM-DD] ingest | <소스 제목>` 엔트리를 append하고 변경 파일 목록을 적는다. 소스 제목은 raw 파일의 H1 헤딩을 사용한다 (H1이 없으면 파일명 슬러그).
